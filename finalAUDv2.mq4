...
OrderOpenTime is 12pm for JPY,and 12pm for EUR, 06am for AUD
*/
        ...
        if (count == 0) {
            ...
            */
            ...
            if ( MathAbs((Open[1]-Close[1])/(High[1]-Low[1])) < 0.5 && MathAbs((High[1]-Open[1])/(Low[1]-Close[1])) > 0.6 ) { //longer top wick than bottom one; Bearish pin
                int sell_ticket=OrderSend(symb, OP_SELL, lots, Bid, 1, Bid+(400*Point), Bid-(1200*Point));
            }
                ...
                int buy_ticket=OrderSend(symb, OP_BUY, lots, Ask, 1, Ask-(400*Point), Ask+(1200*Point));
            }
        }
    }
