...
bearish<-getBearishStock(symbol)
#if a stock is bullish, get its low_vol contracts
lowVol_contracts<-if (!is.null(bullish)) getLowVolContracts(symbol,"C") else if(!is.null(bearish)) getLowVolContracts(symbol,"P")
for (contract in lowVol_contracts[,1]) {

#get the strikes of the selected contracts (using positions)
strike<-lowVol_contracts[,2][match(c(contract),lowVol_contracts[,1])]

resistances<-getPriceLevels(symbol)

#if the stock is going up, u need to establish its location relative to the next resistance
current_price<-tail(get(symbol), n=1)[,6][[1]]


if (current_price < as.numeric(strike) ){#only deal with strikes that are higher than the current price(bullish)
next_resistance<-getNextResistance(resistances,current_price)
...
if (!is.null(bearish)) {
lowVol_contracts<-getLowVolContracts(symbol,"P")
for (contract in lowVol_contracts[,1]) {

#get the strikes of the selected contracts (using positions)
strike<-lowVol_contracts[,2][match(c(contract),lowVol_contracts[,1])]

support_levels<-getPriceLevels(symbol)

#u need the current stock price to determine the upcoming support level
current_price<-tail(get(symbol), n=1)[,6][[1]]

if (current_price > as.numeric(strike) ){#only deal with strikes that are higher than the current price(bullish)
next_support<-getNextSupport(support_levels,current_price)
if (as.numeric(strike) > next_support) {
print(contract)
}
}
}
